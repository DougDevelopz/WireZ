...
@ModuleLoaderInfo(name = "Idle Connections Monitor", description = "Idle Connections Monitor System for database aspects of the plugin", type = ModuleLoaderType.ADDON)
public class IdleConnections extends AbstractModuleLoader implements SystemsWrapper<Integer> {
    ...
    public Integer getElement() {
        ...
        for (Map<String, HikariDataSource> dataSourceMap : MultiDataPoolSetup.grabInstance().getPlayersCurrentDbs().values()) {
            for (int i = 0; i < dataSourceMap.values().size(); i++) {
                ...
                fetchedValue.getAndAdd(pool.getIdleConnections());
            }
        }
